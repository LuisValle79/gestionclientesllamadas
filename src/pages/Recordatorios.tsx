...
const Recordatorios = () => {
  ...
  const [filtroCompletado, setFiltroCompletado] =
    ...
    try {
      ...
      if (!data) {
        ...
        setSnackbar({
          open: true,
          message:
            "Perfil de usuario no encontrado. Se asignó el rol cliente por defecto.",
          severity: "warning",
        });
        return;
      }
  ...
  const fetchRecordatorios = async () => {
    try {
      ...
      let query = supabase
        ...
        .eq("created_by", user.id) // filtramos solo los del usuario
        .order("fecha");
  ...
  const handleOpenDialog = (recordatorio?: Recordatorio) => {
    if (userRole === "cliente") {
      setSnackbar({
        open: true,
        message: "Los clientes no pueden crear ni editar recordatorios",
        severity: "error",
      });
      return;
    }
    if (recordatorio) {
      if (
        userRole === "asesor" &&
        (!user || recordatorio.created_by !== user.id)
      ) {
        setSnackbar({
          open: true,
  ...
  const handleCloseDialog = () => {
    setOpenDialog(false);
    setCurrentRecordatorio({});
    // Restaurar el foco al botón "Nuevo Recordatorio"
    setTimeout(() => {
      const button = document.querySelector(
        'button[aria-label="Crear nuevo recordatorio"]'
      );
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCurrentRecordatorio({
      ...currentRecordatorio,
      [name]: value.slice(0, name === "titulo" ? 100 : 500),
    });
  };
  ...
  const handleClienteChange = (event: SelectChangeEvent<string>) => {
    setCurrentRecordatorio({
      ...currentRecordatorio,
      cliente_id: event.target.value as string,
    });
  };
  ...
  const handleSaveRecordatorio = async () => {
    ...
    if (
      !currentRecordatorio.cliente_id ||
      !currentRecordatorio.titulo ||
      !currentRecordatorio.fecha
    ) {
      setSnackbar({
        open: true,
        message: "Cliente, título y fecha son obligatorios",
        severity: "error",
      });
      return;
    }
    ...
    if (
      ...
      !clientes.find(
        (c) =>
          c.id === currentRecordatorio.cliente_id && c.created_by === user.id
      )
    ) {
      setSnackbar({
        open: true,
        message:
          "No tienes permiso para crear/editar recordatorios para este cliente",
        severity: "error",
      });
      return;
    }
    ...
    try {
      if (isEditing && currentRecordatorio.id) {
        if (
          userRole === "asesor" &&
          currentRecordatorio.created_by !== user.id
        ) {
          setSnackbar({
            open: true,
            message: "No tienes permiso para editar este recordatorio",
  ...
  const handleToggleCompletado = async (id: string, completado: boolean) => {
    ...
    if (userRole === "cliente") {
      setSnackbar({
        open: true,
        message: "Los clientes no pueden actualizar recordatorios",
        severity: "error",
      });
      return;
    }
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: `Recordatorio marcado como ${
  ...
  const handleDeleteRecordatorio = async (id: string) => {
    ...
    if (userRole === "cliente") {
      setSnackbar({
        open: true,
        message: "Los clientes no pueden eliminar recordatorios",
        severity: "error",
      });
      return;
    }
  ...
  return (
    <Container
      ...
      sx={{
        ...
        backgroundImage:
          "linear-gradient(135deg, rgba(33, 150, 243, 0.05), rgba(0, 150, 136, 0.05))",
        borderRadius: 2,
    ...
    >
      ...
      >
        ...
        <Button
          ...
          disabled={
            userRole === null || userRole === "cliente" || clientes.length === 0
          }
        >
          Nuevo Recordatorio
        </Button>
                  ...
                  >
                    ...
                    >
                      <Checkbox
                        ...
                        color="primary"
                        sx={{ p: 0.5 }}
                        aria-label={`Marcar ${recordatorio.titulo} como ${
                          recordatorio.completado ? "pendiente" : "completado"
                        }`}
                        disabled={
                          userRole === "cliente" ||
                          (userRole === "asesor" &&
                      ...
                      <Typography
                        ...
                        sx={{
                          ...
                          fontWeight: 500,
                          fontSize: "1.1rem",
                          textDecoration: recordatorio.completado
                            ? "line-through"
                            : "none",
        ...
        <DialogContent>
          ...
          <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={es}>
            <DateTimePicker
              ...
              value={
                currentRecordatorio.fecha
                  ? new Date(currentRecordatorio.fecha)
                  : null
              }
      ...
      >
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          <Button
            onClick={() =>
              openDeleteDialog && handleDeleteRecordatorio(openDeleteDialog)
            }
            variant="contained"
            color="error"
            sx={{
